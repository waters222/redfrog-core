...
echo "Building test"
env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o ./bin/test ./test/test.go
...
echo "Building proxy-server-arm64"
env CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags $INJECT_PARAMS -a -o ./bin/redfrog-server-arm64 ./proxy_server/server.go
